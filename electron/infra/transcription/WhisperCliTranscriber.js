...
export class WhisperCliTranscriber {
  constructor() {
  ...
  async initialize() {
    try {
      ...
      if (platform === "win32") {
        ...
        if (!this.modelPath) {
          console.warn("WhisperCliTranscriber: No model found.");
        }

        console.log("WhisperCliTranscriber: Initialized (Windows)");
      ...
      console.error("WhisperCliTranscriber: Initialization failed:", error);
      return false;
    }
  }
  ...
  async transcribe(audioBuffer, options = {}) {
    ...
    if (!this.isInitialized) throw new Error("WhisperCliTranscriber not initialized");
    if (!this.modelPath) throw new Error("Whisper model not found");

    try {
      console.log(`WhisperCliTranscriber: Processing ${audioBuffer.length} bytes`);
      ...
      console.error("WhisperCliTranscriber: Transcription failed:", error);
      throw error;
    }
  }
  ...
  async runWhisper(audioFilePath, language) {
    return new Promise((resolve, reject) => {
      ...
      console.log("WhisperCliTranscriber: Command:", whisperPath, args.join(" "));
  ...
  getShortPath(inputPath) {
    ...
    try {
      ...
      console.warn("WhisperCliTranscriber: Short path failed, using original:", error);
      return inputPath;
    }
  }
  ...
  async saveAudioToFile(audioBuffer, filename) {
    ...
    try {
      ...
      console.error("WhisperCliTranscriber: Failed to write temp file:", error);
      throw error;
    }
    ...
    try {
      ...
      console.log(
        `WhisperCliTranscriber: Saved ${stats.size} bytes to ${filepath}`
      );
    } catch (error) {
      console.warn("WhisperCliTranscriber: Failed to stat temp file:", error);
    }
